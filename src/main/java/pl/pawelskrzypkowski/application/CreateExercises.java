...
public class CreateExercises {
	...
	private class LoadingWorkoutWorker extends SwingWorker<Workout, String>{
		public LoadingWorkoutWorker() {
			super();
		}
		...
		@Override
		protected void done() {
			publish("Workout added");
		}

		@Override
		protected Workout doInBackground() throws Exception {
			publish("Start");
			publish("Creating workout");
			...
			publish("Saving workout");
			exampleWorkout.saveWorkout();
			return exampleWorkout;
		}
	}

	private class LoadingExercisesWorker extends SwingWorker<Boolean, String>{
		public LoadingExercisesWorker() {
			super();
		}
		...
		@Override
		protected void done() {
			publish("Exercises added");
		}

		@Override
		protected Boolean doInBackground() throws Exception {
			publish("Start loading");
			publish("Adding shoulders exercises");
			...
			publish("Adding chest exercises");
			...
			publish("Adding biceps exercises");
			...
			publish("Adding legs and rear exercises");
			...
			publish("Adding forearms exercises");
			new Exercise("UGINANIE NADGARSTKÓW PODCHWYTEM W SIADZIE",
					"Ćwiczenie to rozwija wewnętrzną część przedramion-podstawowe dla rozwoju tej partii mięśni. Wyrabia siłę uścisku dłoni. Łapiemy sztangę w siadzie podchwytem, rozstaw dłoni ok. 15 cm(szerszy nadwyręża nadgarstki).Opieramy przedramiona o uda, tak by poza nogi wystawały jedynie nasze dłonie. Pracują tylko nadgarstki. Staramy się, aby zakres ruchu był jak największy i w tym celu pozwalamy w dolnym położeniu na stoczenie się sztangi aż do końców palców - po czym ponownie ściskamy dłoń i zginamy maksymalnie nadgarstek. Ćwiczenie można wykonywać również zastępując sztangę drążkiem wyciągu dolnego, lub sztangielką (jednorącz).",
					new String[] { "grupa mięsni przedramion-zginaczy nadgarstków" }).saveExercise();
			...
			publish("Adding stomach exercises");
	 ...
	 * @throws IOException
	 */
	public boolean loadWorkout() throws ClassNotFoundException, IOException{
		LOG.trace("Start loading workout");
		LoadingWorkoutWorker loadingWorkoutWorker = new LoadingWorkoutWorker();
		loadingWorkoutWorker.execute();
		LOG.trace("Workout loaded");
		return true;
	}
	/**
	...
	public boolean loadExercises() throws IOException {
		LOG.trace("Start loading exercises");
		LoadingExercisesWorker loadingExercisesWorker = new LoadingExercisesWorker();
		loadingExercisesWorker.execute();
		LOG.trace("Exercises loaded");
		return true;
	}

	public void closeWindow() {
		stage.close();
		LOG.trace("Loading files window closed");
	}
}
